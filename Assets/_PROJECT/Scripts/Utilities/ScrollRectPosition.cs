...
public class ScrollRectPosition : MonoBehaviour
{
    ...
    [SerializeField] bool ShowExtraParameters;
    ...
    [ShowIf("ShowExtraParameters")]
    [SerializeField] float yPadding = 0;
    ...
    void Update()
    {
        ...
        if (_selected.transform.parent != contentPanel.transform)
        {
            return;
        }
        ...
        selectedRectTransform = _selected.GetComponent<RectTransform>();
        ...
        float _selectedPositionY = Mathf.Abs(selectedRectTransform.anchoredPosition.y) + selectedRectTransform.rect.height;

        // The upper bound of the scroll view is the anchor position of the content we're scrolling.
        float _scrollViewMinY = contentPanel.anchoredPosition.y;
        // The lower bound is the anchor position + the height of the scroll rect.
        float _scrollViewMaxY = contentPanel.anchoredPosition.y + scrollRectTransform.rect.height;

        // If the selected position is below the current lower bound of the scroll view we scroll down.
        if (_selectedPositionY >= _scrollViewMaxY)
        {
            float newY = _selectedPositionY - scrollRectTransform.rect.height;
        ...
        else if (Mathf.Abs(selectedRectTransform.anchoredPosition.y) <= _scrollViewMinY)
        {
            contentPanel.anchoredPosition = new Vector2(contentPanel.anchoredPosition.x, Mathf.Abs(selectedRectTransform.anchoredPosition.y+yPadding));
        }
